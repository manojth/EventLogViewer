...
namespace EventLogParser
{
    ...
    class EventLogParser : IEventLogParser
    {
            ...
            try
            {
                ...
                using (var fs = new FileStream(fileName, FileMode.Open))
                {
                    ...
                    using (var br = new BinaryReader(fs))
                    {
                        ...
                        totalEvents =totalEvents+ (int)(eventLogHeader.NextIndex - 1);
                        OnAction(String.Format("Found {0} events", totalEvents));
                        // Read the items
                        EventLogEntry e;
                        int cnt = 0;
                        uint offset = eventLogHeader.FooterOffset;
                        while (offset!=0)
                        {
                            ...
                            this.OnFoundRecord(
                                new object[] { 
                                    ...
                                    e.UserSid, 
                                    e.Computername});
                            if (cnt % 200 == 0) this.OnProgress(cnt, Convert.ToInt32(totalEvents));
                            if (offset == 48)
                                break;
                        }
                    }
...
//            }
            ...
            {
                this.OnAction(String.Format("Error Occured! {0} for file name {1}", ex.Message, fileName ));
            }
            return;
        }
